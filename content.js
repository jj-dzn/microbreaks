...
const STRETCHES = [
  {name:'Neck rolls', dur:'30 sec', steps:['Sit or stand tall, shoulders relaxed','Slowly drop your right ear toward your right shoulder','Roll your head forward, chin to chest','Continue to the left side','Complete 3 full slow circles each direction'],
    pose(t){const r=Math.sin(t*PI*0.8)*0.38;return{headTilt:r,headNod:Math.abs(r)*0.25};}},
  {name:'Overhead reach', dur:'30 sec', steps:['Stand with feet hip-width apart','Inhale and raise both arms overhead','Interlace your fingers, palms facing up','Press up gently and hold for 3 breaths','Exhale and slowly lower your arms'],
  ...
  {name:'20-20-20 rule', dur:'20 sec', steps:['Look away from your screen completely','Find something at least 20 feet away','Focus softly on it without straining','Hold your gaze for 20 full seconds','Blink slowly and return to your work'],
    pose(t){const l=ease(sin01(t*0.33));return{headTilt:lerp(0,0.06,l),headNod:lerp(0,-0.08,l)};}},
  {name:'Wrist circles', dur:'20 sec', steps:['Extend both arms in front of you','Make loose fists with both hands','Slowly rotate both wrists outward','Complete 5 circles then reverse direction','Shake out your hands gently to finish'],
    pose(t){const a=t*PI*2;return{shlLang:-PI/8,elbLang:-PI/8+Math.sin(a)*0.3,shlRang:PI+PI/8,elbRang:PI+PI/8+Math.sin(-a)*0.3};}},
  {name:'Shoulder rolls', dur:'30 sec', steps:['Let your arms hang loose at your sides','Lift both shoulders up toward your ears','Roll them slowly backward and down','Feel your shoulder blades squeeze together','Repeat 5 times then roll them forward'],
  ...
  {name:'Spinal twist', dur:'30 sec', steps:['Sit tall with feet flat on the floor','Place your right hand on your left knee','Inhale to lengthen your spine upward','Exhale and gently twist to the left','Hold 3 breaths then switch sides'],
  ...
  {name:'Forward fold', dur:'30 sec', steps:['Stand with feet hip-width apart','Inhale and grow tall through your spine','Exhale and hinge forward at the hips','Let your arms and head hang heavy','Hold 3 breaths then slowly rise back up'],
  ...
  {name:'Chest opener', dur:'30 sec', steps:['Stand or sit upright and tall','Clasp your hands behind your lower back','Squeeze your shoulder blades together','Lift your chest and chin slightly upward','Hold for 3 deep breaths then release'],
  ...
  {name:'Side stretch', dur:'30 sec', steps:['Stand tall with feet hip-width apart','Raise your right arm overhead','Lean slowly to the left side','Feel the stretch along your right side','Hold 3 breaths then switch sides'],
  ...
  {name:'Chin tucks', dur:'20 sec', steps:['Sit or stand with your spine tall','Look straight ahead at eye level','Gently pull your chin straight back','Feel the stretch at the base of your skull','Hold 3 seconds, release, repeat 5 times'],
    pose(t){const tk=ease(sin01(t*0.5));return{headNod:lerp(0,0.22,tk),headTilt:lerp(0,-0.04,tk)};}},
  {name:'Upper back squeeze', dur:'30 sec', steps:['Sit upright with arms at your sides','Bend elbows to 90 degrees','Pull both elbows back behind your body','Squeeze your shoulder blades firmly together','Hold 5 seconds, release, repeat 5 times'],
  ...
  {name:'Temple massage', dur:'20 sec', steps:['Place your fingertips on your temples','Apply gentle circular pressure inward','Move slowly in small circles','Breathe deeply and release jaw tension','Continue for the full 20 seconds'],
  ...
  {name:'Ankle circles', dur:'20 sec', steps:['Sit with your feet lifted off the floor','Point your toes forward gently','Slowly rotate both ankles outward','Complete 5 circles then reverse direction','Flex and point your feet to finish'],
  ...
  {name:'Finger spreads', dur:'20 sec', steps:['Hold both hands out in front of you','Spread your fingers as wide as possible','Hold the spread for 3 seconds','Then curl fingers into a loose fist','Repeat the open-close movement 5 times'],
  ...
  {name:'Hip stretch', dur:'30 sec', steps:['Stand near your desk for balance','Lift your right knee toward your chest','Hold it with both hands gently','Feel the stretch in your hip and glute','Hold 3 breaths then switch legs'],
...
function showOverlay(stretchIndex, male) {
  removeOverlay();
  ...
  doneBtn.textContent = 'Done — I moved';
  ...
  hint.textContent = 'Press Esc or click outside to dismiss';
...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.type === 'SHOW_BREAK_OVERLAY') showOverlay(msg.stretchIndex, msg.male);
